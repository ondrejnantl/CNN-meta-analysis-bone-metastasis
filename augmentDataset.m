function [varargout] = augmentDataset(trainIms,trainLabels,trainStats,trainObjects,trainVertID,trainPatID,catObjCount)
% Popis:
...
%                   Mx1, kde M je počet objektů
% augmentedPatID - vektor obsahující příslušnost objektu k pacientovi s rozměry
%                  Mx1, kde M je počet objektů
% 
% Autor: Ondřej Nantl
% ===================================================================================================================
...
% určení počtu lézí napříč kategoriemi
healthyCount = sum(trainLabels == 0);
lyticCount = sum(trainLabels == 1);
blasticCount = sum(trainLabels == 2);

% určení počtu augmentací pro jednotlivou lézi a celkového počtu augmentací
...
for objectid = 1:size(trainIms,4)
    ...
    % uložení původního objektu do výstupních proměnných
