function [varargout] = getLesions_v4(dataPath)
% Popis:
...
%                     augmentované podoby, rozměr je 32 x 32 x 3 x počet objektů
...
% dataSpineExp2 = im_ozn2(positions_roi(1):positions_roi(2), positions_roi(3):positions_roi(4),...
%                 positions_roi(5):positions_roi(6));
...
% centrování dat
% dataSpine = dataSpine - mean(dataSpine(90:100,190:220,780:820),'all');
% dataSpine = dataSpine - 0.45;
...
% výběr okna CT jasu 
% CADReg(CADReg < -250) = -250;
% CADReg(CADReg > 1250) = 1250;
...
% lesionPercExp2 = zeros(objectCount,3);
...
for cadLesID = 1:objectCount
    ...
    % určení zastoupení typů tkáně v objektu a tvorba labelu                           
    for i = 0:2
...
%         typeExpLesion2 = sum((dataSpineExp2(CADConnMat == cadLesID) == i));
%         lesionPercExp2(cadLesID,i+1) = typeExpLesion2/numel(CADConnMat(CADConnMat == cadLesID));
    end
    [maxPerc1, maxLPerEIter1] = max(lesionPercExp1(cadLesID,2:3));
%     [maxPerc2, maxLPerEIter2] = max(lesionPercExp2(cadLesID,2:3)); 
