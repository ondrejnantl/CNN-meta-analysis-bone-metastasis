%% načtení dat k vykreslení pro všechny modely krome binárního
...
avgXMulti = avgMulti.avgX;
avgYMulti = avgMulti.avgY;
avgXLytic = avgLytic.avgX;
avgYLytic = avgLytic.avgY;
avgXBlastic = avgBlastic.avgX;
avgYBlastic = avgBlastic.avgY;
avgXBinary = avgBinary.avgX;
avgYBinary = avgBinary.avgY;

%% samotné vykreslení
figure;
for i = 1:3
    % vykreslení ROC křivky multiclass modelu
    ROCm{i} = plot(avgXMulti(:,i),avgYMulti(:,i),'--');
    hold on
...
ROCbl = plot(avgXBlastic,avgYBlastic);
hold on
...
ROCbi = plot(avgXBinary,avgYBinary);
hold on
